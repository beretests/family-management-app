  ...
  try {
    ...
    const taskRows = newSelections.map((selection) => {
      ...
      return {
        ...
        completion_check_text_snapshot: template.completionCheckText,
        evidence_type_snapshot: template.evidenceType,
        is_undesirable: template.undesirableScore >= 3,
        points_possible: template.basePoints,
        requires_evidence_snapshot: template.requiresEvidence,
